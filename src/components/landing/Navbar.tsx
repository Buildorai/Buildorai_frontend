...
export default function Navbar() {
  ...
  useEffect(() => {
    ...
    const setupObserver = () => {
      ...
      const observerOptions = {
        root: null,
        rootMargin: "-20% 0px -60% 0px", // Trigger when section is in the upper middle area
        threshold: 0,
      };
    ...
    // Especially important for Next.js page transitions
  ...
  // Sync active section with hash on navigation to home
  useEffect(() => {
    if (pathname === "/") {
      ...
      if (hash) {
        ...
        // If no hash, we're likely at the top
      ...
      <div className={`flex items-center justify-between rounded-full border border-white/10 bg-background/60 p-2 backdrop-blur-xl transition-all duration-500 ${scrolled ? "px-4 md:px-6 py-1.5 shadow-2xl shadow-primary/5" : "px-6 md:px-8 py-2"
        ...
        <Link href="/" className="flex items-center gap-3 text-xl font-bold tracking-tight text-white transition-opacity hover:opacity-80">
          <Image
            ...
            width={44}
            height={44}
            className="h-9 w-auto object-cover md:h-12"
            priority
          />
              ...
              {navLinks.map((link) => {
                ...
                return (
                  <Link
                    ...
                    className={`rounded-2xl border px-6 py-3 text-center text-lg font-medium transition-all ${isActive
